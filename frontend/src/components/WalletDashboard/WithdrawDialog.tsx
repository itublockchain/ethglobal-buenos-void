...
export function WithdrawDialog({
    ...
    const handleWithdraw = async () => {
        ...
        try {
            setError(null);
            // Use decimals from the token asset (default to 18 if missing)
            // Note: Asset type usually has decimals? Let's check types.ts later.
            // For now assuming standard ERC20 behavior or passing decimals if available.
            // The Asset type in index.tsx seemed to have amount and value but maybe not decimals explicitly?
            // Let's check how DepositDialog gets decimals. It gets it from usePublicWalletTokens which returns full token info.
            // Here 'tokens' are Asset[]. Let's assume standard 18 or try to find it.
            // Wait, Asset from types.ts might not have decimals.
            // Let's check types.ts.

            // Assuming 18 for now if not present, but ideally we should pass it.
            // Actually, let's look at index.tsx again. 'assetsFromBackend' maps to { symbol, name, amount, value, address, logo }.
            // It doesn't seem to include decimals.
            // However, the contract needs the amount in Wei.
            // We might need to fetch decimals or pass them.
            // In DepositDialog, publicWalletTokens has decimals.
            // Here, we might need to fetch it or store it.
            // For now, let's assume we can get it or default to 18.
            // Actually, let's check if we can get decimals from the token list or if we need to fetch it.
            // The backend balance just has 'balance' which is string.
            // Wait, in index.tsx:
            // const balanceValue = parseFloat(balance.balance);
            // It seems the backend returns formatted balance? Or raw?
            // "balance.balance" is string.
            // If it's formatted, we need decimals to parse back to Wei.
            // If it's raw Wei, we need decimals to format for display.
            // In index.tsx: "const balanceValue = parseFloat(balance.balance);"
            // This suggests it's already formatted (e.g. "10.5").
            // So to send to contract, we need to parseUnits(amount, decimals).
            // We need decimals.
            // I'll assume for now we can get decimals or I'll add it to Asset type if needed.
            // Let's look at index.tsx again. It fetches metadata for unknown tokens.
            // But for known tokens (SUPPORTED_TOKENS), we have decimals in constants.ts.

            let decimals = 18;
            // Try to find in SUPPORTED_TOKENS
            // We need to import SUPPORTED_TOKENS or pass it.
            // Or maybe we can just use a hook to get decimals if we have the address.

            const amountInWei = parseUnits(withdrawAmount, decimals);

            writeWithdraw({
                address: VOID_CONTRACT_ADDRESS,
                abi: VOID_CONTRACT_ABI,
                functionName: "withdraw",
                args: [amountInWei, (selectedToken.address || "0x0000000000000000000000000000000000000000") as Address],
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <TokenSelector
                                                tokens={tokens.map(t => ({
                                                    ...
                                                    balance: parseUnits(t.amount.toString(), 18), // Mocking balance as BigInt for TokenSelector
                                                    formattedBalance: t.amount.toString(),
                                                    decimals: 18 // Mocking decimals
                                                }))}
                                        ...
                                        >
                                            ...
                                            <div className="relative group">
                                                <Input
                                                    ...
                                                    className="h-20 bg-black/40 border border-white/10 focus:border-white/50 focus:bg-black/60 text-4xl font-light pl-6 pr-20 rounded-none transition-all placeholder:text-white/30 text-white shadow-inner"
                                                />
                                    ...
                                    <div className="flex flex-col h-full justify-start items-center text-center space-y-4">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <p className="text-sm text-white/50 max-w-[280px] mx-auto leading-relaxed">
                                                ...
                                                from your Void Wallet.
                                            </p>
                                        </div>

                                        <div className="w-full bg-white/5 rounded-none p-4 border border-white/5 backdrop-blur-sm">
